...
def print_image_windows(
    ...
    """Show the native printer dialog and print one centered, aspect-fit page."""
    if os.name != "nt":
...
class NavigationBar(ttk.Frame):
    ...
    def __init__(self, master: tk.Widget, viewer: "CrowEyesImageViewer") -> None:
        ...
        for icon, tip, command in (
            ("previous", "이전 폴더", viewer.go_folder_back),
            ("next", "다음 폴더", viewer.go_folder_forward),
            ("up", "상위 폴더", viewer.go_parent_folder),
            ("refresh", "현재 폴더 새로고침 (F5가 아닌 폴더 갱신)", viewer.refresh_folder),
        ):
        ...
        viewer._breadcrumb_frame = ttk.Frame(self, style="Breadcrumb.TFrame", padding=(7, 3))
        ...
        viewer._top_search.pack(side=tk.LEFT, ipady=4)
...
class PlaylistPanel(ttk.Frame):
    ...
    def __init__(self, master: tk.Widget, viewer: "CrowEyesImageViewer") -> None:
        ...
        selectors.pack(fill=tk.X, pady=(8, 8))
        ...
        ttk.Label(selectors, textvariable=viewer._playlist_summary_var, style="Section.TLabel", anchor="w").pack(fill=tk.X)
...
class PrintPreview(tk.Toplevel):
    ...
    def do_print(self) -> None:
        if self.viewer._print_with_options(
            self.orientation_var.get(), self.paper_var.get(), self.scale_var.get(), parent=self,
        ):
            self.destroy()
...
class CrowEyesImageViewer(tb.Window):
    def __init__(self, start_path: Optional[str] = None) -> None:
        ...
        self._playlist_summary_var = tk.StringVar(value="PLAYLIST 0개")
    ...
    def _update_responsive_layout(self, width: int) -> None:
        ...
        if compact_level != self._last_responsive_compact:
            ...
            if hasattr(self, "_brand_label"):
                self._brand_label.configure(text="CrowEyes" if width >= 960 else "")
    ...
    def _populate_list_names_only(self) -> None:
        ...
        mode = LIST_MODE_LABELS.get(self.list_mode, self.list_mode)
        sort = SORT_LABELS.get(self.sort_by, self.sort_by)
        self._playlist_summary_var.set(f"{shown}/{total}개 · {mode} · {sort} {arrow}")
        ...
        if self.list_mode == "icons" and self._grid_inner is not None:
            ...
            for kind, p, image_idx in items:
                if kind == "image" and image_idx is not None:
                    ...
                    key = f"{kind}:{p}"
                    label_text = ".." if kind == "parent" else self._short_folder_name(p.name)
                    folder_image = self._icons["folder_large"]
                    lbl = tk.Label(
                    ...
                    lbl.bind("<ButtonRelease-1>", self._on_playlist_button_release)
                    lbl.bind("<Double-Button-1>", lambda _e, path=p: self._navigate_playlist_folder(path))
        ...
        for row, (kind, p, image_idx) in enumerate(items):
            if kind != "image":
                text = "..  상위 폴더" if kind == "parent" else p.name
                w.insert("", tk.END, iid=f"folder:{row}", text=text, image=self._icons["folder"])
                continue
    ...
    def _on_folder_icon_click(self, _event, path: Path) -> None:
        ...
        self._selected_folder_path = path
        self._highlight_icon_selection()
    ...
    def _on_playlist_button_release(self, _event=None):
    ...
    def _on_playlist_double_click(self, event):
        item = self._playlist_item_at_event(event)
        if item is not None and item[0] in {"parent", "folder"}:
    ...
    def _on_list_select(self, _event=None) -> None:
        ...
        if isinstance(w, tk.Listbox):
            ...
            if sel:
                ...
                if image_indices:
                    ...
                    idx = active[2] if active[0] == "image" else next(reversed(sorted(image_indices)))
        ...
        if sel:
            ...
            if image_indices:
                ...
                idx = focused[2] if focused is not None and focused[0] == "image" else max(image_indices)
                assert idx is not None
                self._selection_anchor = idx
                self.goto_index(idx, from_list=True)
                self._queue_thumbs_priority()
    ...
    def _update_navigation_state(self) -> None:
        if len(self._navigation_buttons) >= 4:
            self._navigation_buttons[0].configure(state=tk.NORMAL if self._folder_history_index > 0 else tk.DISABLED)
            self._navigation_buttons[1].configure(
                state=tk.NORMAL if self._folder_history_index + 1 < len(self._folder_history) else tk.DISABLED
            )
            can_up = self.current_folder is not None and self.current_folder.parent != self.current_folder
            self._navigation_buttons[2].configure(state=tk.NORMAL if can_up else tk.DISABLED)
            self._navigation_buttons[3].configure(state=tk.NORMAL if self.current_folder else tk.DISABLED)
    ...
    def _update_breadcrumb(self) -> None:
        ...
        if folder is None:
            ttk.Label(frame, text="이 PC", style="Breadcrumb.TLabel").pack(side=tk.LEFT)
            return
        chain = list(reversed((folder,) + tuple(folder.parents)))
        for position, part in enumerate(chain):
            if position:
                ttk.Label(frame, text="›", style="Breadcrumb.TLabel").pack(side=tk.LEFT, padx=3)
            label = part.name or part.drive or str(part)
            button = ttk.Button(
                frame, text=label, style="Breadcrumb.TButton",
                command=lambda target=part: self.navigate_to_folder(target),
            )
    ...
    def _apply_theme(self) -> None:
        ...
        style.configure("Breadcrumb.TLabel", background=t["panel_alt"], foreground=t["muted"], font=("Segoe UI", 9))
        style.configure(
            "Breadcrumb.TButton", background=t["panel_alt"], foreground=t["fg"],
    ...
    def _visible_playlist_items(self) -> List[Tuple[str, Path, Optional[int]]]:
        ...
        if folder is not None:
            ...
            if parent != folder:
                items.append(("parent", parent, None))
    ...
    def _print_with_options(
        self, orientation: str, paper: str, scale_mode: str, parent: Optional[tk.Widget] = None,
    ) -> bool:
    ...
    def _show_loading_state(self, filename: str) -> None:
        ...
        self.canvas.create_text(cw / 2, ch / 2 + 24, text=f"{filename} 불러오는 중…",
                                fill=secondary, font=("Segoe UI", 10))
        self.status.configure(text="이미지 불러오는 중…")
