...
def inspect_svg(path: Path) -> Tuple[str, Tuple[float, float]]:
    ...
    for element in root.iter():
        ...
        for key, value in element.attrib.items():
            ...
            if local_key in {"href", "src"} and candidate.startswith(("http://", "https://", "//", "file://")):
                raise FormatSupportError("외부 네트워크 또는 파일 리소스가 포함된 SVG는 표시하지 않습니다.")
            if local_key == "style" and re.search(r"url\(\s*['\"]?(?:https?:|//|file:)", candidate):
                raise FormatSupportError("외부 리소스가 포함된 SVG는 표시하지 않습니다.")
        if local_tag == "style" and element.text:
            if re.search(
                r"(?:url\(\s*['\"]?|@import\s+['\"]?)(?:https?:|//|file:)",
                element.text, re.I,
            ):
                raise FormatSupportError("외부 리소스가 포함된 SVG는 표시하지 않습니다.")
...
def render_svg(path: Path, target_width: int, target_height: int) -> Tuple[Image.Image, Tuple[float, float]]:
    ...
    try:
        ...
        raise FormatSupportError(f"SVG 렌더링에 실패했습니다: {exc}") from exc
...
def print_image_windows(
    ...
    try:
        ...
        print_rect = fit_print_rect(image.size, (printable_w, printable_h))
...
class PlaylistPanel(ttk.Frame):
    ...
    def __init__(self, master: tk.Widget, viewer: "CrowEyesImageViewer") -> None:
        ...
        viewer._sort_dir_btn = ttk.Button(
            selectors, textvariable=viewer._sort_dir_text_var, width=2,
            command=viewer.toggle_sort_dir, style="Tool.TButton",
        )
        viewer._sort_dir_btn._croweyes_style = "Tool.TButton"  # type: ignore[attr-defined]
        viewer._sort_dir_btn.pack(side=tk.LEFT, padx=(5, 0))
        ToolTip(viewer._sort_dir_btn, "정렬 방향 전환 · ▲ 오름차순 / ▼ 내림차순")
...
class CrowEyesImageViewer(tb.Window):
    ...
    def print_current_image(self) -> None:
        ...
        try:
            ...
            printed = print_image_windows(self._image_for_print, owner, path.name)
            self.status.configure(
            ...
            messagebox.showerror("인쇄 실패", detail, parent=self)
