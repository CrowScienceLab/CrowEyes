...
try:
    ...
    C2paReader = None  # type: ignore[assignment]
...
CROWEYES_THEMES = {
    ...
    "white_light_pink": {
        "bg": "#FFF8FB", "panel": "#FFFFFF", "panel_alt": "#FDECF3",
        "canvas_bg": "#FFF4F8", "border": "#E8B8CB", "fg": "#3C2731",
        "muted": "#80616F", "accent": "#E783AA", "accent_hi": "#B84F79",
        "accent_button": "#B54A73", "accent_pressed": "#963452",
        "select": "#F9D7E5", "select_fg": "#3C2731",
        "danger": "#B93657", "toolbar": "#FFF0F6", "status": "#FCE5EE",
        "button": "#F8D5E3", "button_active": "#F2BDD2", "button_pressed": "#EBA5C0",
        "button_fg": "#432832", "button_highlight": "#FFFFFF", "button_shadow": "#9A4C69",
        "placeholder": "#A98292", "font": ("Segoe UI", 10),
        "font_title": ("Segoe UI Semibold", 11),
    },
}
...
def c2pa_ai_probability(path: Path) -> Tuple[int, str]:
    """Map verified C2PA provenance evidence to a five-step AI confidence.

    This is an evidence-strength scale, not a statistical pixel classifier:
    0=no manifest, 25=unreadable/invalid manifest, 50=valid provenance without
    an AI declaration, 75=AI declaration with failed validation, and 100=valid
    AI provenance. No EXIF heuristics or visual model guesses are used.
    """
    if C2paReader is None:
        return 0, "C2PA 검증 모듈 없음"
    reader = None
    try:
        reader = C2paReader.try_create(path)
        if reader is None:
            return 0, "C2PA 정보 없음"
        active_manifest = reader.get_active_manifest()
        if not active_manifest:
            return 25, "C2PA 매니페스트를 읽을 수 없음"
        manifest_text = json.dumps(active_manifest, ensure_ascii=False).lower()
        ai_declared = any(
            marker.decode("ascii") in manifest_text
            for marker in _C2PA_AI_SOURCE_MARKERS
        )
        valid = bool(reader.is_valid)
        if ai_declared and valid:
            return 100, "유효한 C2PA 생성형 AI 출처 선언"
        if ai_declared:
            return 75, "C2PA AI 선언 있음 · 검증 경고 또는 실패"
        if valid:
            return 50, "유효한 C2PA 출처 · AI 생성 선언 없음"
        return 25, "C2PA 출처 있음 · 검증 실패"
    except Exception as exc:  # c2pa-python exposes format-specific exception subclasses
        detail = str(exc).lower()
        if "not supported" in detail or "unsupported" in detail or "no claim" in detail:
            return 0, "C2PA 정보 없음"
        return 25, "C2PA 정보를 확인했으나 해석할 수 없음"
    finally:
...
class CrowEyesIconSet:
    """Large, antialiased Crow-family command glyphs and brand mark."""
    ...
    def __init__(self, root: tk.Misc, colors: dict) -> None:
        ...
        for name in toolbar_icons:
            self.images[name] = self._render(name, 24)
        self.images["eye"] = self._load_brand_eye()
        self.images["eye_large"] = self._load_brand_eye(88)
        self.images["folder_large"] = self._render("folder", 52)
        self.images["computer_large"] = self._render("computer", 52)
        self.images["app"] = self._load_brand_eye(36)
    ...
    def _load_brand_eye(self, size: int = 48) -> ImageTk.PhotoImage:
        path = resource_path("assets", "icons", "croweyes-1.9.png")
        try:
            with Image.open(path) as opened:
    ...
    def _render(self, name: str, size: int, app_badge: bool = False) -> ImageTk.PhotoImage:
        ...
        # CrowLink/CrowPack-inspired visual language: generous optical scale,
        # rounded geometry, strong white/silver structure and cyan accents.
        line = max(6, int(size * 0.085 * scale))
        s = scale
...
class CrowEyesImageViewer(tb.Window):
    ...
    def show_preferences(self) -> None:
        ...
        ttk.Radiobutton(
            frame, text="성인용 컨텐츠 차단", variable=safety_var, value=MODE_EXPLICIT,
    ...
    def _show_blocked_state(self, error: bool = False) -> None:
        ...
        if error:
            ...
            title = "보호된 이미지"
            body = "성인용 컨텐츠로 판단되어 화면 표시를 차단했습니다."
            color = t["accent_hi"]
    ...
    def _update_info(self) -> None:
        ...
        ai_probability, c2pa_basis = c2pa_ai_probability(path)
        data = [
            ("AI 이미지 확률 · C2PA", f"{ai_probability}%"),
            ("판별 근거", c2pa_basis),
            ("파일 형식", format_text),
            ("파일 크기", size_text),
            ("해상도", resolution_text),
